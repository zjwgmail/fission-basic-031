...
func wireApp(bootstrap *conf.Bootstrap, confServer *conf.Server, confData *conf.Data, business *conf.Business, logger log.Logger) (*kratos.App, func(), error) {
	...
	httpServer := server.NewHTTPJobServer(confServer, logger, helpCodeService, initService, uploadService)
	activityJob := biz.NewActivityJob(activityInfoRepo, logger, bootstrap, redisService)
	...
	cronService := service.NewCronService(activityJob, resendRetryJob, resendJob, feishuReportJob, emailReportJob, drainageJob, logger)
	...
	userRemindUsecase := biz.NewUserRemindUsecase(confData, logger, userRemindRepo, waMsgService)
	userRemindService := service.NewUserRemindService(logger, userRemindUsecase, helpCodeService, activityInfoUsecase, clusterClient)
	retryService := service.NewRetryService(logger, officialRallyUsecase, unOfficialRallyUsecase, msgUsecase, clusterClient)
	cronServer := server.NewCronServer(confData, cronService, taskService, userRemindService, retryService, logger)
